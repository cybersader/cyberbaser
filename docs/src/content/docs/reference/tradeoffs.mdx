...
- **Auth for elevated/maintainer roles: GitHub OAuth vs. self-hosted (Forgejo) vs. multi-provider?** Default: GitHub OAuth for the current Decap path. Note: contribution itself is gated by a maintainer-set trust curve + moderation queue, not a forced contributor account.
