...
**Lesson for cyberbaser:** Quartz proves that an Obsidian-aware static publisher is viable and not that hard to build. The gap Quartz doesn't fill is *exactly* the contribution layer — which means cyberbaser can learn from Quartz's translation decisions while layering Decap (or similar) on top.
