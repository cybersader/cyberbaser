...
**Knowledge-ops rule:** the **canonical KB is the docs site** (`docs/src/content/docs/` — problem, principles, vision, architecture, research). The `.claude/` layer orients agents and points there. Every meaningful research, architecture, product-direction, real acceptance, or implementation wave also gets one dated `docs/src/content/docs/agent-context/zz-log/YYYY-MM-DD-slug.mdx` entry in the same session. When a decision gets locked, update `.claude/` (FOCUS, 41-QUESTIONS-RESOLVED, PROJECT_CONTEXT if direction changed) **in the same session**. Current truth without chronology loses the reasoning trail; chronology without current orientation sends fresh agents in an old direction. Docs preflight fails when `FOCUS.md` is newer than the latest dated `zz-log`.

**UI-review delivery rule:** when the maintainer needs to inspect a coherent interface, make it available through the normal project experience: deploy it when separately authorized, link it from the canonical docs with one explicit launch command, or both. Do not hand over an unexplained local port or disconnected mockup. Keep privileged product origins separate when the security model requires it, but coordinate the review from docs and record it in the dated log.
...
bun run review:owner-alpha  # real authenticated proposal-review fixture
