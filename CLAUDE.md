...
The immediate milestone is owner self-dogfooding on Cyberbase: one maintainer switches between reader and owner contexts across three to five normal and adversarial correction attempts. `@cyberbaser/correction` is the no-I/O exact-anchor and single-splice primitive used by that series, not a shipped editor, generalized adapter API, endpoint, automatic writer, or account-free product path. The seven-field local form is an instrument, not the eventual reader or rich-authoring UI. Build only the smallest improvement justified by observed dogfood friction. The preserved five-reader, one-independent-owner protocol is optional later, before any unfamiliar-reader or independent-owner usability claim.
