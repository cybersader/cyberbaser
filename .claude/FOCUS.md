...
**Current:** **v1 Build.** The current product interpretation is an **owner-controlled change boundary** around a version-controlled knowledge base: decide what may publish, accept bounded proposals against pinned source, preserve untouched bytes, classify integrity and trust, and leave final control with the owner. Authoring tools are replaceable spokes: reader controls, trusted rich browser/WYSIWYG/CMS-like tools, forge editors, local Markdown tools, and agents may present different experiences. External spokes converge on exact proposals and owner review; the owner keeps direct local authoring. GitHub, Actions, Pages, and pinned Quartz are the working dogfood stack, not the product essence. The internal agentic correction dry run and private four-command operator rehearsal completed on 2026-07-28. Together they prove only that one bounded candidate and the prepare/render/decision path can complete locally without writing source. The immediate milestone is now [owner self-dogfooding](/cyberbaser/research/owner-self-dogfood/): one maintainer switches between reader and owner contexts across three to five normal and adversarial Cyberbase attempts. `@cyberbaser/correction` supplies exact quote anchoring and fail-closed single-splice preparation/application as a no-I/O primitive; neither the package, private kit, nor seven-field local instrument is a shipped editor, generalized adapter contract, writer, or account-free product path. The five-reader independent protocol is preserved for later if stronger usability claims need it.
...
Last updated: 2026-07-30 (owner self-dogfood made immediate; five-reader protocol deferred unchanged; no new R-number)
...
- The account-free product path is unshipped. The internal agentic dry run and private operator rehearsal completed, but no owner self-dogfood attempt has completed the full reader-to-live loop. No ordinary-reader or independent-owner evidence exists. Owner dogfood counts toward none of the deferred pilot thresholds and does not close Q09.
...
**Immediate milestone: run owner self-dogfooding on Cyberbase.** The sequence now:
...
4. **NEXT: run the owner self-dogfood series:** three to five OD attempts covering one normal correction, signed-out mobile handoff, stale source, ambiguous quote, and owner rejection. One maintainer may switch roles across devices. Record device-specific observations and preserve every blocked run. This is operational and mechanical evidence only; Q09 remains open.
5. **Let observed friction decide the smallest next change.** Fix only a burden or safety defect the owner series exposes. It does not select or validate a rich editor, automatic writer, hosted console, or generalized adapter. The preserved five-reader, one-independent-owner protocol remains optional before stronger usability claims.
6. **Make enforcement honest without breaking local authoring (Q10):** OFM is report-only, trust is decision-only, and the maintainer's direct push path still needs compatible validation.
7. **Close the remaining concrete gaps:** verify contributor attribution (Q08), freeze the URL contract and slug-diff gate (Q06), and decide the operating model for a maintainer who does not use GitHub.
8. **Preserve federation as long-term evidence:** keep the five-origin fixture bounded and local. Seek a concrete need from an independently operated peer before extracting any provisional profile or conformance work.
...
- Calling the concierge, seven-field study form, local review card, or `@cyberbaser/correction` primitive a shipped editor, generalized adapter contract, endpoint, automatic writer, or account-free contribution product
