...
8. **Next up — track 2 (site quality):** (a) **pipeline link-check** (Quartz silently ships ~5.7% unresolved links; the 28-page `_attachments` class is the fidelity-break corpus's first entry); (b) the 75 emoji **folder** renames (free for wikilinks, URLs unfrozen — this is the churn window) + a theme pass; (c) flip `ofm-check` to required (observation window must include reading logs, not just checkmarks — see R17). **Track 3:** agent-contributor gates (~2 d). Later: URL freeze + slug-diff gate; Quartz v5 evaluation (pin stays v4.5.2 until re-measured).
