...
6. **Next up (pre-publish list):** (a) the maintainer authors `publish.yml` (folder walk — only they can); (b) projection wiring: selector → lowercase/alias → frontmatter pre-flight → Quartz; (c) the 2 lint renames + 4 frontmatter kill-switch files + `Templates/` ignore; (d) derived-path lint + output link-check (Quartz hides broken links; truth is 5.7%); (e) v4-vs-v5 pick; then first Pages deploy of the vault.
