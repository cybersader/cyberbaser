...
8. ~~Tracks 2 + 3~~ **SHIPPED 2026-07-26 (R18)**: `@cyberbaser/linkcheck` (first real number: 1145 broken → **779** after the `RelativeFolderLinks` fix; attachment class 248 → 7), theme + the reported explorer-scroll bug (root-caused to upstream `overscroll-behavior`, verified with headless wheel events), and `@cyberbaser/trust` landed in the vault as **decision-only** labeling. Five packages, **151 tests green**.
9. **Next up:** (a) **the 669 remaining missing-page links** — inherited source-vault debt (Notion-export leftovers, GUID paths), not pipeline damage; needs a cleanup pass or a documented ignore list, and it's the fidelity-break corpus's real content; (b) **the identity gap** — `agents:` is empty because there's no bot account yet; agent auto-merge is inert until one exists; (c) flip `ofm-check` to required; (d) **maintainer decision pending: the 43 emoji folder renames** (73 path-qualified wikilinks would break — the earlier "free" claim was wrong; options: all / none / published-only, URL churn window open until the slug-diff gate); (e) URL freeze + slug-diff gate; (f) Quartz v5 evaluation (pin stays v4.5.2 until re-measured).
