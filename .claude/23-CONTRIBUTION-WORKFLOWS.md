...
- The missing account-free path is Q09. Owner self-dogfooding is immediate: one maintainer uses separate reader and owner contexts across three to five normal and adversarial Cyberbase attempts. The five-reader, one-independent-owner protocol is deferred until stronger usability claims need it. Neither experiment can select the final reader UI, rich editor, generalized proposal record, or adapter API.
