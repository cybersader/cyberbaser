...
- Decided and installed in dogfood: emitted-site URL continuity under R23. The current verbatim path-derived URLs remain the cumulative baseline; every previous canonical URL must remain canonical or directly redirect-covered. Cyberbase now runs the pinned checker before Pages publication.
...
- Open: whether and when to select a rich trusted-contributor surface that can conform through a deliberate pinned-base exact-proposal adapter · general maintainer operating model · backlink compute · asset hosting · Tier 2/3 rendering strategy.
