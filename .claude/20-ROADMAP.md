...
Ordered work packages, written 2026-08-02 and aligned 2026-08-10: **WP1** two-stage trusted ledger trigger → **WP2** container packaging → **WP3** Forgejo phase 1 (deployment adapter + disposable one-time-mirrored acceptance; authority cutover is a separate explicit decision) → **WP4** Q09 intake lanes behind one shared `@cyberbaser/proposal` contract → **WP5** management console (configure/delegate, never store identity) → **WP6** identity seam stays open. **WP1 implementation is complete and hermetically tested. WP2 implementation and mechanical acceptance are complete within the documented Linux/amd64 evidence boundary. WP3 phase one is complete:** the rejected same-UID host runner was replaced by unprivileged run-scoped job containers, the isolation probe passed, the controlled PR and exact owner-alpha Save completed on Forgejo 16.0.2, peak temporary storage was 172,289,041 bytes, and exact cleanup left no labelled resources or run roots. **WP4's shared contract, Forgejo Lane A adapter, carrier-neutral local queue, account-free Lane B adapter/app, disabled Quartz form, and local-only sibling-service deployment bundle are implemented and mechanically tested at their stated layers.** The no-R-number amendment selects the v1 local filesystem queue and sibling service. Offered-lane work, live Lane A integration, public Lane B exposure, and human evidence remain next. WP1 live installation and any Forgejo authority cutover remain separate authorization boundaries; the ledger evidence clock has not started. The plan's standing constraints remain locked.
...
- [x] Complete WP3 phase-one acceptance (2026-08-10): strict adapter and provider dispatch, Forgejo-only non-default HTTPS port support, disposable one-time seed, checksum-bound runner daemon, unprivileged run-scoped job containers, exact two-source mount allowlist, isolation probe, private checkout, controlled PR gates, exact owner-alpha Save, fixed run/job binding, HTTPS witness, 4 GiB guard, unchanged authoritative seed, and manifest-bound cleanup all passed on Forgejo 16.0.2. Peak combined temporary storage was 172,289,041 bytes. Authority cutover remains separate.
- [x] Implement the shared `@cyberbaser/proposal` contract: one pinned existing Markdown source, one canonical exact splice, inert identity evidence, receiver-verified trust handoff, no I/O or authority.
- [x] Implement and hermetically test the Forgejo Lane A adapter against injected Forgejo metadata and real local Git objects. It is read-only, no-authority, not live-validated, not installed, not linked, and not offered.
- [x] Implement the WP4 local queue and account-free Lane B mechanics: strict anonymous derivation, finite carrier-neutral filesystem queue, separate optional sibling app, disabled Quartz form, and local-only internal-network OCI bundle. The no-R-number amendment selects the v1 local queue and sibling service; all layers remain proposal-only and no-authority.
...
- [x] Verify contributor attribution through the GitHub fork flow (R24/Q08). Cross-repository PR #7 carried one `Benthediver99` commit; the normal owner squash merge retained that mapped account, name, and noreply address as authoritative commit author. Run `31551432560` and Pages deployment `5861172637` succeeded. Broader page credit, anonymous/pseudonymous attribution, multi-commit behavior, other forges, and trust-history computation remain open under challenge 07.
...
- [ ] Decide the operating model for a maintainer who does not use GitHub: hosted, one-click self-hosted, or local-first. Owner self-dogfooding tests one local workflow but does not choose the general product model. The 2026-08-12 controlled local Iroh fixture passed forced direct-IP and local-relay exact-proposal transfer, deterministic reconnect continuation, zero-byte duplicate handling, and content-identical Lane B replay collapse. This selects neither Iroh nor the custom fixture ALPN for production. AT Protocol remains an optional public identity/signed-record/carrier/discovery spoke; no substrate or offered lane is selected.
