...
3. **Contributors should not need git or a forced account** — the live GitHub path does not yet satisfy this. Account-free Lane B mechanics are implemented but disabled, unexposed, and human-untested, so they do not yet create an offered path. The preserved concierge pilot is deferred until stronger unfamiliar-reader or independent-owner claims need it.
...
6. **Research before implementation** — no feature without a written justification first. The exact-correction primitive and shared proposal contract are narrow libraries; the seven-field pilot is a study instrument; the Lane B queue/app/form are disabled local mechanics. None by itself is a shipped editor, offered intake lane, source writer, or product path.
